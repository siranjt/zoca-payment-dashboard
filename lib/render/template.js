...
function buildReport(data) {
  ...
  const sec5 = data.section5_pointers || [];
  const sec6 = data.section6_actions || {};
  const sec7 = data.section7_systemic || {};
  const sec8 = data.section8_gaps || [];
  const sec9 = data.section9_evidence || {};
  ...
  const doc = new Document({
    ...
    sections: [{
      ...
      children: [
        ...
        ...((sec8.items || sec8 || []).map(t => numberedItem(t))),

        h1("Section 9 · Evidence trail and methodology"),
        ...((sec9.methodology_paragraphs || []).map(t => para(t))),
        h2("9.1 Evidence trail"),
        ...((sec9.evidence_trail || []).map(t => bullet(t))),
